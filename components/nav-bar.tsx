...
import { Gift, Layers, Swords, Package } from "lucide-react";
...
const navItems = [
  { href: "/", label: "Gacha", icon: Gift },
  { href: "/collection", label: "Sammlung", icon: Layers },
  { href: "/battle", label: "Kampf", icon: Swords },
];

export function NavBar({ freePulls = 0, maxPulls = 10 }: NavBarProps) {
    ...
    <header className="sticky top-0 z-50 border-b border-border bg-white/80 backdrop-blur-md shadow-sm">
      <div className="container mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-2">
          <ODISLogo className="text-odis-blue" />
          <span className="font-bold text-lg hidden sm:block text-odis-dark">
            Open Data Gacha
          </span>
        </Link>
        ...
        {/* Free pulls display */}
        <div className="flex items-center gap-2 px-3 py-1.5 bg-secondary rounded-lg shadow-sm">
          <Package className="w-4 h-4 text-odis-blue" />
          <span className="font-mono font-bold text-sm">
            <span
              className={cn(
                freePulls === maxPulls ? "text-emerald-600" : "text-foreground",
              )}
            >
              {freePulls}
            </span>
            <span className="text-muted-foreground">/{maxPulls}</span>
          </span>
        </div>
      </div>
    </header>
